...
from .meta_upsampler import MetaUpsampler
...
class UNetBlock(nn.Module):
    ...
    def __init__(
        ...
        # metadata small network initialization
        self.upsampler = MetaUpsampler(self.inner_shape)
        (C, H, W) = self.inner_shape

        # concatenation only occurs at C=383 --> 384 for the ViT input
        if C == 383:
            self.inner_shape = (C+1, H, W)
    ...
    def forward(self, x, meta=None):
        ...
        # r : (N, C_inner, H, W)
        (y, r) = self.conv(x)
        (C, H, W) = self.get_inner_shape()

        if meta != None and C == 383:
            processed_meta = self.upsampler(meta)
            y = torch.cat([y, processed_meta], dim=1) # concatenation
